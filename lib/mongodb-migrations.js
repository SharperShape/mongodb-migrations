(function() {
  var Migrator, Promise, _, defaultLog, fs, migrationStub, mkdirp, mongoConnect, path, repeatString,
    slice = [].slice;
  ...
  Migrator = (function() {
    ...
    Migrator.prototype._run = function(direction, done, progress) {
      ...
      runOne = (function(_this) {
        return function() {
          var context, fn, id, isCallbackCalled, migration, migrationDone, skipReason, timeoutId;
          if (i >= l) {
            return allDone();
          }
          migration = m[i];
          ...
          migrationDone = function(res) {
            ...
            if (res.status === 'ok') {
              return insertPromises.push(Promise.fromCallback(function(cb) {
                var id;
                id = migration.id;
